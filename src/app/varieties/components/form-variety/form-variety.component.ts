...
@Component({
  selector: 'form-variety',
  templateUrl: './form-variety.component.html'
})


export class FormVarietyComponent implements OnInit, OnDestroy {
  ...
  deleteVariety() {
    this.VarietiesService.delete(this.guid).subscribe(data => {
      this.gotoList();
  ...
  updateVariety() {
    this.VarietiesService.put(this.guid, this.variety).subscribe(data => {
      this.gotoList();
    }, error => alert(error.error));
  }
  
  gotoList() {
    this.router.navigate([environment.FORM_LIST_VARIETIES]);
  }
