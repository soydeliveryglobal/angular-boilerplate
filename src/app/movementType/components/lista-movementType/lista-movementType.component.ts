import { PageEvent } from '@angular/material/paginator';
import { Paginador } from './../../../navigation/paginador';
import { ResponseAll } from './../../../core/models/ResponseAll';
import { I18nServiceService } from './../../../core/services/i18n/i18n-service.service';
import { TranslateService } from '@ngx-translate/core';
import { MovementType } from './../../../core/models/movementtype';
import { Component, OnInit,AfterViewInit, ViewChild } from '@angular/core';
import { Router } from '@angular/router';
import { environment } from 'src/environments/environment';
import { MovementTypeService } from 'src/app/core/services/abm/movementTypes.service';
import { LoginService } from 'src/app/core/services/login.service';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';

@Component({
  selector: 'lista-movementtype',
  templateUrl: './lista-movementtype.template.html',
  styleUrls: ['./lista-movementtype.scss'],
})
export class ListaMovementTypeComponent extends Paginador implements OnInit {
  @ViewChild(MatSort) sort: MatSort;
  MovementTypes:MatTableDataSource<MovementType>;
  environment = environment;


  displayedColumns: string[] = ['name','description','createdOn','updatedOn','actions'];

  ngOnInit() {
   // this.consultar();
  }

  private consultar() {
    this.movementtypeService.getAll().subscribe((res: ResponseAll) => {
      this.MovementTypes = new MatTableDataSource(res.data);
      this.MovementTypes.sort = this.sort;
    });
  }

  constructor(
    private movementtypeService: MovementTypeService,
    private router: Router,
    private login: LoginService,
    private translate: TranslateService,
    private i18nService: I18nServiceService
  ) {
    super();
    this.i18nService.localeEvent$.subscribe((locale) => {
      this.translate.use(locale);
    });

    this.consultar()
  }

  deleteMovementType(guid: string) {
    this.router.navigate([
      environment.FORM_CRUD_MOVEMENTTYPE,
      guid,
      environment.MODO_DELETE,
    ]);
  }

  updateMovementType(guid: string) {
    this.router.navigate([
      environment.FORM_CRUD_MOVEMENTTYPE,
      guid,
      environment.MODO_UPDATE,
    ]);
  }

  movementTypeDetail(guid: string) {
    this.router.navigate([
      environment.FORM_CRUD_MOVEMENTTYPE,
      guid,
      environment.MODO_DISPLAY,
    ]);
  }

  createMovementType() {
    this.router.navigate([
      environment.FORM_CRUD_MOVEMENTTYPE,
      0,
      environment.MODO_CREATE,
    ]);
  }


  public  pageEvent(page: PageEvent ){   
    if (!page){
        this.pagina = environment.PAGINA_INICIAL;  
    }else{
        this.pagina = Number(page.pageIndex);
        this.pageSize = page.pageSize 
    }
    this.consultar();
  } 


  
  
}
